...
def join_us_shipping_zones_from_shipstaton_export(shipment_export_csv: Path, zone_export_csv: Path, output_csv: Path,
                                                  join_key_a='ship_id', join_key_b='Shipment ID',
                                                  zone_column_name='Zone'):
    shipments = pd.read_csv(shipment_export_csv)
    zones = pd.read_csv(zone_export_csv, encoding='ISO-8859-1')[[join_key_b, zone_column_name]]
...
def clean_carriers(shipment_csv: Path, output_csv: Path):
    ...
    shipments.loc[(shipments['carrier'].isna() | (shipments['carrier'] == ''))
                  & shipments['service_lower'].str.contains('ups'), 'carrier'] = 'UPS'
...
def set_international_shipping_zones(shipment_csv: Path, output_csv: Path,
                                     carrier_column_name='Carrier', country_column_name='Country'):
    ...
    with (Path(__file__).resolve().parent / 'int_shipping_zones_country_codes.json').open() as file:
        zones_json = json.load(file)

    shipments['zone'] = shipments.apply(lambda row: find_zone(row['country'], row['carrier'])
                                        if pd.isna(row['zone']) or row['zone'] == '' else row['zone'], axis=1)
    shipments.to_csv(output_csv, index=False)


if __name__ == '__main__':
    processed_path = (CURRENT_DIR := Path(__file__).resolve().parent) / 'clean_shipping_data.csv'
    join_us_shipping_zones_from_shipstaton_export(CURRENT_DIR / 'out.csv', CURRENT_DIR / 'zones.csv', processed_path)
